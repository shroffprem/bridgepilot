...
export default function Loans() {
  ...
  const openCases = loans.filter(l => ['open', 'overdue'].includes(l.status));
  ...
  const LoanTable = ({ rows, showStatus = true }) => (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/50 border-b border-border text-xs text-muted-foreground uppercase tracking-wide">
                ...
                {showStatus && <th className="text-center px-4 py-3 font-medium">Status</th>}
              </tr>
            </thead>
            <tbody>
              {rows.map(l => {
                ...
                return (
                  <tr key={l.id} className="border-b border-border last:border-0 hover:bg-muted/30 cursor-pointer" onClick={() => navigate(`/loans/${l.id}`)}>
                    ...
                    <td className={`px-4 py-3 text-right font-semibold ${l.status === 'overdue' ? 'text-red-600' : ''}`}>{formatINR(outstanding)}</td>
                    <td className={`px-4 py-3 text-right text-xs font-mono ${days > 7 ? 'text-red-500' : 'text-muted-foreground'}`}>{days}</td>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-syne font-bold text-xl">Cases</h2>
        ...
        <Tabs defaultValue="open">
          <TabsList className="mb-4">
            <TabsTrigger value="open">
              Open Cases
              {openCases.length > 0 && (
                <span className="ml-2 bg-primary text-primary-foreground rounded-full text-xs px-1.5 py-0.5 leading-none">{openCases.length}</span>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="open">
            <LoanTable rows={openCases} showStatus={false} />
          </TabsContent>
          ...
          <TabsContent value="all">
            ...
            <LoanTable rows={filtered} showStatus={true} />
          </TabsContent>
        </Tabs>
      )}
    </div>
